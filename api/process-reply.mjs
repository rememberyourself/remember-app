...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!replies || replies.length < 1) return res.status(200).json({ status: 'no replies' });
    ...
    if (conversationText.trim().length < 20) return res.status(200).json({ status: 'too short' });
    ...
    await supabase.from('checkins').update({ conversation_analysis: convAnalysis }).eq('id', checkinId);
    console.log(`✅ Conversation analysis saved for ${checkinId}`);

    // Push to client if coach replied
    if (replyFrom === 'coach') {
      try {
        await fetch(`${siteUrl}/api/send-push`, {
          method: 'POST',
